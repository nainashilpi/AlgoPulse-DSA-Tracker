...
import { User, Mail, Lock, Code2, UserPlus, Activity, Loader2 } from "lucide-react";
      ...
      >
        <div className="bg-white/[0.01] backdrop-blur-3xl border border-white/5 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-8 relative z-10">
            ...
            <h1 className="text-lg font-black text-white italic tracking-tighter uppercase">New_Identity</h1>
            <div className="flex items-center gap-2 mt-1">
              <Activity size={10} className="text-cyan-500 animate-pulse" />
              <span className="text-[7px] font-black text-slate-600 tracking-[0.4em] uppercase italic">Protocol: Initialize</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-white/5 pl-11 pr-4 py-3.5 rounded-xl text-white text-[10px] font-bold outline-none focus:border-cyan-500/30 transition-all placeholder:text-slate-800 tracking-widest uppercase"
            ...
            <div className="relative group">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-white/5 pl-11 pr-4 py-3.5 rounded-xl text-white text-[10px] font-bold outline-none focus:border-cyan-500/30 transition-all placeholder:text-slate-800 tracking-widest"
                onChange={(e) => setFormData({ ...formData, password: e.target.value })}
              />
            </div>

            <div className="relative group">
              <Code2 size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700 group-focus-within:text-cyan-500 transition-colors" />
              <input
                type="text"
                required
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-4 bg-cyan-600 hover:bg-cyan-500 text-black rounded-xl font-black text-[10px] tracking-[0.2em] uppercase italic transition-all flex items-center justify-center gap-2 active:scale-95 mt-4 disabled:opacity-50"
            >
              {isLoading ? (
                <>INITIALIZING... <Loader2 size={14} className="animate-spin" /></>
              ) : (
                <>Confirm_Node <UserPlus size={14} /></>
              )}
            </button>
          </form>
